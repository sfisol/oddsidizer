...
impl Odds {
    pub fn to_american(self) -> Result<i32, ConversionError> {
        match self {
            ...
            Odds::Decimal(decimal) => decimal_to_american(decimal),
            Odds::Fractional { num, den } => fractional_to_american(num, den),
        }
    }

    pub fn to_fractional(self) -> Result<(u32, u32), ConversionError> {
        match self {
            Odds::American(inner) => american_to_fractional(inner),
            Odds::Decimal(decimal) => decimal_to_fractional(decimal),
    ...
    pub fn to_decimal(self) -> Result<Decimal, ConversionError> {
        match self {
            Odds::American(inner) => american_to_decimal(inner),
    ...
    pub fn to_decimal_str(self) -> Result<String, ConversionError> {
        ...
        Ok(format!("{:.2}", decimal.round_dp(2)))
    }
}

#[cfg(test)]
// --- Helper for comparing Decimals in tests ---
fn assert_decimal_eq(a: Decimal, b: Decimal) {
...
mod tests {
    ...
    fn assert_decimal_ok_eq(a: Result<Decimal, ConversionError>, b: Decimal) {
        assert!(a.is_ok());
    ...
    fn test_from_odds_to_fractional_tuple() {
        ...
        assert_eq!(american.to_fractional(), Ok((2, 3)));
    ...
    fn test_to_fractional_str() {
        ...
        #[cfg(not(feature = "lookup"))]
        assert_eq!(Odds::American(-150).to_fractional_str().unwrap(), "2/3");
